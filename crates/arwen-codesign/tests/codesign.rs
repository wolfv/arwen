...
/// The strongest correctness anchor we have without a macOS machine: for
/// binaries produced by Apple's real `codesign --sign -`, re-signing them
/// with the same identifier must reproduce the original file **byte for
/// byte** — the entire file, padding included, not just structurally.
///
/// This breaks the circularity of validating the signer against our own
/// `verify()`: the expected bytes here were generated by Apple, so a shared
/// wrong assumption between signer and verifier cannot make this pass.
///
/// These fixtures were signed with the ad-hoc `-s -` path, which is exactly
/// what this crate emulates. (Other fixtures such as `xmllint`/`patchelf`
/// were signed by a different `codesign` invocation that reserves ~18 KiB of
/// slack for a CMS certificate; that padding size is version-specific and
/// deliberately not reproduced — see the note on bit-for-bit compatibility
/// in the crate docs.)
#[rstest]
...
fn resigning_reproduces_codesign_output_byte_for_byte(#[case] name: &str) {
    let original = fixture(name);
    // Use the identifier codesign embedded, so the only variable left is our
    // signature generation.
    let identifier = verify(&original).unwrap()[0].identifier.clone();

    let resigned = adhoc_sign(original.clone(), &AdhocSignOptions::new(&identifier)).unwrap();

    assert_eq!(
        resigned.len(),
        original.len(),
        "{name}: re-signed size differs from codesign output"
    );
    assert!(
        resigned == original,
        "{name}: re-signed bytes differ from codesign output (first diff at {:?})",
        resigned.iter().zip(&original).position(|(a, b)| a != b)
    );
}

/// The signature *content* (the meaningful SuperBlob bytes, excluding the
/// trailing zero padding whose size is version-specific) must be byte-for-byte
/// identical to codesign's for every ad-hoc fixture. This is a slightly weaker
/// but broader companion to the full-file test above.
#[rstest]
#[case::bzip2("conda-repackaged/bzip2")]
#[case::openssl("conda-repackaged/openssl")]
#[case::zstd("conda-repackaged/zstd")]
#[case::ruby("conda-repackaged/ruby")]
fn signature_content_matches_codesign(#[case] name: &str) {
    let original = fixture(name);
    let identifier = verify(&original).unwrap()[0].identifier.clone();
    let resigned = adhoc_sign(original.clone(), &AdhocSignOptions::new(&identifier)).unwrap();

    let (orig_off, orig_content) = signature_content(&original);
    let (mine_off, mine_content) = signature_content(&resigned);
    // Everything before the signature (header, load commands, code) must match.
    assert_eq!(
        &original[..orig_off],
        &resigned[..mine_off],
        "{name}: code region before the signature differs"
    );
    assert_eq!(
        orig_content, mine_content,
        "{name}: signature content differs from codesign"
    );
}

/// Return `(signature_offset, signature_content_bytes)` for a thin
/// little-endian 64-bit Mach-O binary, where the content is the meaningful
/// SuperBlob (its declared length), excluding any trailing allocation padding.
fn signature_content(data: &[u8]) -> (usize, &[u8]) {
    let le32 = |o: usize| u32::from_le_bytes(data[o..o + 4].try_into().unwrap());
    let be32 = |o: usize| u32::from_be_bytes(data[o..o + 4].try_into().unwrap());
    assert_eq!(le32(0), 0xfeed_facf, "expected a thin LE 64-bit Mach-O");
    let ncmds = le32(16);
    let mut offset = 32usize;
    for _ in 0..ncmds {
        let cmd = le32(offset);
        let cmdsize = le32(offset + 4) as usize;
        if cmd == 0x1d {
            // LC_CODE_SIGNATURE
            let sig_off = le32(offset + 8) as usize;
            let content_len = be32(sig_off + 4) as usize; // SuperBlob.length
            return (sig_off, &data[sig_off..sig_off + content_len]);
        }
        offset += cmdsize;
    }
    panic!("binary has no LC_CODE_SIGNATURE");
}
